...
class PrimaryImageSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Dynamically create thumbnail fields based on "options" JSONfield in
        # in Tier model.
        try:
        ...
        # Add field with link to original image
        if orig_img_link == True:
            self.fields['original_image'] = serializers.SerializerMethodField()

        # Add field with signed-timestamped link to binary image
    ...
    # Using easy-thumbnails, generates thumbnails and generates links for them
    ...
    # Generates binary image and generates link to that image
    def get_bin_image(self, obj):
        request = self.context.get('request')
        img = binarize(obj.image, obj.image.name, 128)
        return request.build_absolute_uri(img)
    
    # Generates timestamped and signed link to binary image,
    # expiration settings are taken from User-Tier configuration
...
class ExpiringLinkSerializer(serializers.Serializer):
    ...
    def get_url(self, obj):
        ...
        time by entering value in expiration_time field and sending POST.
        It can be changed by writing to_representation method, to only
        accept pre validated data, but then there won't be an autogenerated
        link on GET request.
        '''
        request = self.context.get('request')
